...
#pragma endregion
                ...
                HRESULT result = dev->GetDeviceContext()->Map(((DX11Mesh*)this->gameModel->GetMeshes()[0])->GetVertexBuffer(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
                if (FAILED(result)) {
                    LOG_ERROR("ResourceMapping failed - HRESULT: " << std::hex << result);
                    return OS_RESOURCEMAPPING_FAILED;
                }

                // Get a pointer to the data in the vertex buffer.
                OVertex* verticesPtr = (OVertex*)mappedResource.pData;

                // Copy the data into the vertex buffer.
                memcpy(verticesPtr, (void*)vertices, (sizeof(OVertex)* this->vertexCount));

                // Unlock the vertex buffer.
                dev->GetDeviceContext()->Unmap(((DX11Mesh*)this->gameModel->GetMeshes()[0])->GetVertexBuffer(), 0);
